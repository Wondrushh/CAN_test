...
#define TX_CAN1_BUFFER_SIZE 64 //max 255
#ifdef _DUAL_CAN_ACTIVATE
#define TX_CAN2_BUFFER_SIZE 64 //max 255
...
volatile uint8_t ExtendEnable = 0;
#endif
...
HAL_StatusTypeDef CANbus_Init(CAN_HandleTypeDef *hcan1){
 ...
 /*----------CAN1 init------------*/
  defineCAN1 = hcan1; //copy pointer to local variable

  if (HAL_CAN_ActivateNotification(defineCAN1, CAN_IT_TX_MAILBOX_EMPTY)!= HAL_OK){
    // Activate interrupt CAN_IT_TX_MAILBOX_EMPTY
    return HAL_ERROR;
  }
  if (HAL_CAN_ActivateNotification(defineCAN1, CAN_IT_LAST_ERROR_CODE)!= HAL_OK){
    // Activate interrupt CAN_IT_LAST_ERROR_CODE
    return HAL_ERROR;
  }
  if (HAL_CAN_ActivateNotification(defineCAN1, CAN_IT_ERROR)!= HAL_OK){
    // Activate interrupt CAN_IT_ERROR
    return HAL_ERROR;
  }
  ...
  defineCAN2 = hcan2; //copy pointer to local variable

  if (HAL_CAN_ActivateNotification(defineCAN2, CAN_IT_TX_MAILBOX_EMPTY)!= HAL_OK){
    // Activate interrupt CAN_IT_TX_MAILBOX_EMPTY
    return HAL_ERROR;
  }
  if (HAL_CAN_ActivateNotification(defineCAN2, CAN_IT_LAST_ERROR_CODE)!= HAL_OK){
    // Activate interrupt CAN_IT_LAST_ERROR_CODE
    return HAL_ERROR;
  }
  if (HAL_CAN_ActivateNotification(defineCAN2, CAN_IT_ERROR)!= HAL_OK){
    // Activate interrupt CAN_IT_ERROR
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef CANbus_FilterInit(){
  if (defineCAN1 == NULL){   //if defineCAN is not empty
    return HAL_ERROR;
  }
  ...
  for ( uint8_t i= 0;  i < 14; i++) {
    if(__CAN1_FILTERLIST[i][5] == 1){   //if filter is activate
...
#ifdef _DUAL_CAN_ACTIVATE        /*--CAN2 init--*/
  if (defineCAN2 == NULL){   //if defineCAN is not empty
    return HAL_ERROR;
  }
  for ( uint8_t i= 0;  i < 14; i++) {
    if(__CAN2_FILTERLIST[i][5] == 1){   //if filter is activate
      tmpFilterDef.FilterBank = i;      //number of filter bank
...
HAL_StatusTypeDef CANbus_ExtendedFilterInit(){
  ...
  tmpFilterDef.FilterIdHigh = 0x0;      //ID1
...
HAL_StatusTypeDef CAN1bus_SendData(uint16_t msgID, size_t msgSize, uint16_t msgCnf, uint8_t aData[]){
  if ( HAL_CAN_GetTxMailboxesFreeLevel(defineCAN1) != 0){ //Check that all the Tx mailboxes are not full
    ...
    for (uint8_t i = 0; i < msgSize; ++i) { //copy payload
      tmpData[i]=aData[i];
    }
  ...
  } else {  //write msg to buffer
    if (((TXCAN1buffHead + 1) % TX_CAN1_BUFFER_SIZE) == TXCAN1buffTill){ //if buffer is full
      return HAL_BUSY;
    }else{

      uint8_t tmpTXbufftill = TXCAN1buffTill;
      while(tmpTXbufftill != TXCAN1buffHead){   //scan whole buffer
        if(TXCAN1buff[tmpTXbufftill].msgID == msgID){ //if buff id agree with new id
          if((msgCnf & CANdef_ALLMSG) == 0){ //if selected send only new msg
...
HAL_StatusTypeDef CAN2bus_SendData(uint16_t msgID, size_t msgSize, uint16_t msgCnf, uint8_t aData[]){
  if ( HAL_CAN_GetTxMailboxesFreeLevel(defineCAN2) != 0){ //Check that all the Tx mailboxes are not full
    ...
    if (((TXCAN2buffHead + 1) % TX_CAN2_BUFFER_SIZE) == TXCAN2buffTill){ //if buffer is full
      return HAL_BUSY;
    }else{

      uint8_t tmpTXbufftill = TXCAN2buffTill;
      while(tmpTXbufftill != TXCAN2buffHead){
        if(TXCAN2buff[tmpTXbufftill].msgID == msgID){ //if buff id agree with new id
          if((msgCnf & CANdef_ALLMSG) == 0){ //if selected send only new msg
...
*/
void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *hcan){//Interrupt, when sand all msg in mailbox
  if (hcan->Instance == defineCAN1->Instance){
    ...
    if(TXCAN1buffHead != TXCAN1buffTill){//add msg from buffer to mailbox
      while((TXCAN1buff[TXCAN1buffTill].msgConf & TXBUFF_NO) != 0){//if message is unnecessary
        TXCAN1buffTill = (TXCAN1buffTill + 1) % TX_CAN1_BUFFER_SIZE;  //increment TXCAN1buffTill
        if(TXCAN1buffHead == TXCAN1buffTill){//možná že je zbytečná
          return;
        }
      }
      CAN_TxHeaderTypeDef tmpTxHeader;
    ...
    if(TXCAN2buffHead != TXCAN2buffTill){//add msg from buffer to mailbox
      while((TXCAN2buff[TXCAN2buffTill].msgConf & TXBUFF_NO) != 0){//if message is unnecessary
        TXCAN2buffTill = (TXCAN2buffTill + 1) % TX_CAN2_BUFFER_SIZE;  //increment TXCAN2buffTill
        if(TXCAN2buffHead == TXCAN2buffTill){//možná že je zbytečná
          return;
        }
      }
      CAN_TxHeaderTypeDef tmpTxHeader;
...
void HAL_CAN_ErrorCallback(CAN_HandleTypeDef *hcan) {   //global interrupt error state

  if (hcan->ErrorCode == HAL_CAN_ERROR_ACK) {   //if missed a ACK bit in error state
    HAL_CAN_AbortTxRequest(hcan, CAN_TX_MAILBOX0);  //abort all msg
